...
export function decode_message(bytes) {
    ...
    const ret = wasm.decode_message(ptr0, len0);
    if (ret[2]) {
 ...
 * @returns {Uint8Array}
 */
export function encode_query(id, name, qtype) {
    const ptr0 = passStringToWasm0(name, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.encode_query(id, ptr0, len0, qtype);
    if (ret[3]) {
 ...
 * @returns {string}
 */
export function format_message(bytes) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.format_message(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
